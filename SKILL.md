...
Use generic discovery as the default for ordinary recruitment forms. It reads visible controls, accessible labels, placeholders, headings, repeated records, control types, and stable current-DOM locators before applying the common resume dictionary. A site adapter is only an optional, value-free structural cache for a previously verified page; it is never a prerequisite. Use `--generic` to force the generic path even when a cached adapter exists. Do not encode coordinates, copied values, or a separate mapping algorithm for every employer. The bundled profiles cover several known frameworks, but their presence does not limit the supported websites.
...
3. **Build a read-only mapping.** Start with generic discovery of the visible page. If a verified hostname adapter exists, it may provide a structural shortcut; if it is absent or its fingerprint fails, continue with generic discovery instead of failing. Match labels plus section/record context to `resume.json`, preferring accessible labels, stable data attributes, and current-DOM locators over generated classes or coordinates. Report current and required record counts; do not click “添加” during preview. Report each field as `ready`, `needs-confirmation`, `filled-skip`, `missing`, `manual`, `blocked`, or `skip`.
...
node "<skill-root>/backend/scripts/map-resume.mjs" --resume "<resume JSON path>" --url "<unknown recruitment URL>" --generic
